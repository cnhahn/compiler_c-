...
//creates a root node all functions and statements 
//will be childeren of program is root node made.
astree* new_parseroot(){
   return new astree (TOK_ROOT, {0, 0, 0}, "<<ROOT>>");
}

astree* new_function( astree *ident, astree *param, astree *block){
   int token = TOK_FUNCTION;
   if(block->symbol == ';'){
      token = TOK_PROTOTYPE;
   }
   astree* funk = new astree(token, ident->lloc , "function");
   funk->adopt(ident, param, block);
   return funk;
}
